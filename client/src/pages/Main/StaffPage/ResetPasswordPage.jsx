...
const ResetPasswordPage = () => {
  ...
  const handleSearch = () => {
    if (searchTerm === '') {
      ...
      axios.get(`${URL_API}/emailcollection?searchTerm=${searchTerm}`, config)
        .then((response) => {
          setSearchResult(response.data.listing);
        })
        .catch((error) => {
  ...
  const handleConfirmedResetPassword = async () => {
    ...
    // console.log(`Clicked passwordreset for user with id ${selectedUser}`);
    try {
      await axios.post(`${URL_API}/resetpassword/${selectedUser}`, null, config);
    ...
    <>
      {isSuccess && (
        <div className='bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative z-50' role='alert'>
          <strong className='font-bold'>รีเซ็ตรหัสผ่าน</strong>
      ...
      {showModal && (
        <div className="fixed z-10 inset-0 overflow-y-auto top-72">
          <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <h3 className="text-lg leading-6 font-medium text-gray-900">ยืนยันการรีเซ็ตรหัสผ่าน</h3>
                <div className="mt-2">
                  <p className="text-sm text-gray-500">
                    คุณแน่ใจหรือว่าต้องการรีเซ็ตรหัสผ่านสำหรับผู้ใช้นี้หรือไม่?
                  </p>
                </div>
              </div>
      ...
      <div className='absolute w-0 top-24 -z-10 m-0 left-1/4'>
        <div className='relative m-10 bg border w-max py-10 rounded-lg shadow pr-56 px-10'>
          <div className='flex'>
              ...
              <table className="table-auto w-full">
                <thead className="min-w-full divide-y divide-gray-200">
                  <tr>
                    ...
                    <th className="w-1/3 px-6 py-3 text-left text-m text-gray-500 uppercase tracking-wider">รหัสนิสิต</th>
                    <th className="w-1/3 px-6 py-3 text-left text-m text-gray-500 uppercase tracking-wider">อีเมล</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {searchResult.map((user) => (
                    <tr key={user.id}>
                      <td className="px-6 py-4 whitespace-normal text-sm text-gray-900 w-1/4 ">{user.profile?.thaiFirstName}</td>
                      <td className="px-6 py-4 whitespace-normal text-sm text-gray-900 w-1/4 ">{user.education?.stuID}</td>
                      <td className="px-6 py-4 whitespace-normal text-sm text-gray-900 w-1/4 ">{user.profile?.email}</td>
                      <td className="px-6 py-4 whitespace-normal text-sm text-gray-900 w-1/4 ">
                        <button
                          className='bg-red-500 hover:bg-red-700 duration-100 text-white font-bold py-2 px-4 -mx-7 rounded'
