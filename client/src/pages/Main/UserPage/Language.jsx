...
import ModalLang from '../../../components/modal/main/ModalLang';
...
const Language = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const data = response.data.language_proficiency;
        setFormData(data);
        console.log(data)
      } catch (error) {
        console.log(error);
  ...
  const handleAddLanguage = async (newLang) => {
    setLoading(true);
    try {
      const newId = `${newLang.language}-${Date.now().toString()}`;
      ...
      setFormData((prevState) => ({ ...prevState, [newId]: newLang }));
      setShowModal(false);
      setIsSuccess(true)
    } catch (error) {
      console.log(error)
  ...
  const handleEditLanguage = async (updatedLang) => {
    ...
    try {
      await axios.put(
        ...
        {
          language_proficiency: {
            [updatedLang.id]: updatedLang,
          },
        }, config
      );
      setFormData((prevState) => {
        const newState = { ...prevState };
        newState[updatedLang.id] = updatedLang;
        return newState;
      });
      setEditingLang(null);
      setShowModal(false);
  ...
  const handleDeleteLanguage = async (langId) => {
    setLoading(true);
    try {
      await axios.delete(`${URL_API}/collection/${uid}/language_proficiency/${langId}`, config);
      setFormData((prevState) => {
        const newState = { ...prevState };
            ...
            <table className="w-full">
              <thead>
                <tr>
                  ...
                  <th className="px-4 py-2">วันเวลาที่สอบ</th>
                  <th className="px-4 py-2">ชื่อแบบทดสอบที่วัดระดับความรู้</th>
              ...
              {formData && Object.keys(formData).map((key, index) => (
                <tbody key={key}>
                  <tr className={`${index % 2 === 0 ? 'bg-gray-100' : 'bg-white'} align-top`}>
                    <td className="border px-4 py-2">{formData[key].language}</td>
                    <td className="border px-4 py-2">{formData[key].date}</td>
                    <td className="border px-4 py-2">{formData[key].name}</td>
                    <td className="border px-4 py-2">{formData[key].score}</td>{ }
                    {role !== 'teacher' && (
                      <td className="border-y px-4 py-2">
                        <button
                          onClick={() => {
                            setEditingLang({ ...formData[key], id: key });
                            setShowModal(true);
                          }}
                    ...
                    {role !== 'teacher' && (
                      <td className="border-y border-r px-4 py-2">
                        <button
                          onClick={() => handleDeleteLanguage(key)}
