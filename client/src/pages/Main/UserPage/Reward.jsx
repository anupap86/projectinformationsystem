...
import ModalReward from '../../../components/modal/main/ModalReward';
...
const Rewards = () => {
    ...
    const handleEdit = (type, id) => {
        ...
        setEditSession({ ...sessionToEdit, id });
        setShowModal(true);
        setModalType(type);
    }
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (response.data) {
                    ...
                    const workSessionsData = rewardData.work ? Object.entries(rewardData.work).map(([id, data]) => ({ ...data, id })) : [];
                    const studySessionsData = rewardData.study ? Object.entries(rewardData.study).map(([id, data]) => ({ ...data, id })) : [];
    ...
    const handleAddData = async (type, newId, data) => {
        ...
        try {
            ...
            const response = await axios.post(`${URL_API}/collection/${uid}/reward/${type}`, newSession, config);
    ...
    const handleEditData = async (type, id, data) => {
        setLoading(true);
        try {
            const response = await axios.put(`${URL_API}/collection/${uid}/reward/${type}/${id}`, data, config);
    ...
    const handleDeleteData = async (type, id) => {
        setLoading(true);
        try {
            const response = await axios.delete(`${URL_API}/collection/${uid}/reward/${type}/${id}`, config);
                        ...
                        <table className='min-w-full table-auto'>
                            <thead className='bg-gray-100'>
                                <tr>
                                    ...
                                    <th>ระดับรางวัล</th>
                                    <th>ประเภทรางวัล</th>
                                    <th>ผู้ที่มอบรางวัล</th>
                                    <th>ชันปีที่ได้รางวัล</th>
                                    <th>วันที่ได้รับ</th>
                                </tr>
                            </thead>
                            <tbody>
                                {studySessions.map((session, index) => (
                                    <tr key={session.id}>
                                        <td style={{ maxWidth: '900px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{session.reward_name}</td>
                                        <td style={{ maxWidth: '900px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{session.level}</td>
                                        <td style={{ maxWidth: '900px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{session.type}</td>
                                        <td style={{ maxWidth: '900px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{session.issuer}</td>
                                        <td style={{ maxWidth: '900px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{session.year}</td>
                                        <td style={{ maxWidth: '900px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{session.received_date}</td>
                                            {role !== 'teacher' && (
